...
class CP4IntegrationInstall(object):
    ...
    def updateSecret(self, icp4iInstallLogFile):
        ...
        secret_update_oc = '{"ocpPassword": "' + self.ocpassword.strip() + '"}'
        ...
        secret_update_pn = '{"adminPassword": "' + self.cp4iPassword.strip() + '"}'
        TR.info(methodName, "secret_update_pn %s"%secret_update_pn)
        response = self.secretsmanager.update_secret(SecretId=self.pnSecret,SecretString=secret_update_pn.strip())
    ...
    def main(self,argv):
        ...
        try:
            ...
            with open(logFilePath,"a+") as icp4iInstallLogFile:
                ...
                self.installOCPFlag = environ.get('INSTALL_OCP_FLAG')
                ...
                self.pullSecret = "/ibm/pull-secret.txt"
                ...
                TR.info(methodName, "InstallOCPFlag %s" %self.installOCPFlag)
                if(self.installOCPFlag=="True"):
                    ...
                    send_email = ("sudo ./cp4i-deployment/email-notify.sh  'Openshift Cloud Platform v4.8.22' 'completed' " + self.Namespace + " '" + self.NotificationEmail + "' '' ")
                
                    try:
                        e_process = Popen(send_email,shell=True,stdout=icp4iInstallLogFile,stderr=icp4iInstallLogFile,close_fds=True)
                        stdoutdata,stderrdata=e_process.communicate()
                    except CalledProcessError as e:
                        TR.error(methodName, "ERROR return code: %s, Exception: %s" % (e.returncode, e), e)
                        raise e 
                else:
                    TR.info(methodName, "Skipped installing OCP & OCS as retry flag is $retry")
                
                
                if(self.password=="NotProvided"):
                    install_cp4i = ("sudo ./cp4i-deployment/cp4i-install.sh  -n " +  self.Namespace + " -k " + self.apiKey + " -i " + self.CP4IVersion +
                                " -1 " + self.APILM + " -2 " + self.AIDB + " -3 " + self.AIDE + " -4 " + self.OD + " -5 " + self.AR +
                                " -6 " + self.MQ + " -7 " + self.ES + " -8 " + self.GW + " -9 " + self.HST + " -e '" + self.NotificationEmail +"' | tee -a cp4i-logs.txt")
                else:
                    install_cp4i = ("sudo ./cp4i-deployment/cp4i-install.sh  -n " +  self.Namespace + " -k " + self.apiKey + " -i " + self.CP4IVersion +
                                    " -1 " + self.APILM + " -2 " + self.AIDB + " -3 " + self.AIDE + " -4 " + self.OD + " -5 " + self.AR +
                                    " -6 " + self.MQ + " -7 " + self.ES + " -8 " + self.GW + " -9 " + self.HST + " -p " + self.password + " -e '" + self.NotificationEmail+ "' | tee -a cp4i-logs.txt")
                ...
                if(self.password=="NotProvided"):
                    ...
                    try:
                        self.cp4iPassword = check_output(['bash','-c', get_cp4i_password_cmd]).strip()
                ...
                try:
                    ...
                    TR.error(methodName, "updateSecret : Exception with message %s" %e)
            #endWith    
        ...
        message = "CP4I Install using CoNatI4CP - " + status
        
        send_email_e = ("sudo ./cp4i-deployment/email-notify.sh " + message + " " + email_status + " " + self.Namespace + " '" + self.NotificationEmail + "' '' ")
                
        try:
            end_process = Popen(send_email_e,shell=True,stdout=icp4iInstallLogFile,stderr=icp4iInstallLogFile,close_fds=True)
            stdoutdata,stderrdata=e_process.communicate()
        except CalledProcessError as e:
            TR.error(methodName, "ERROR return code: %s, Exception: %s" % (e.returncode, e), e)
            raise e
