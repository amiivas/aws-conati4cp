...
export install_status=$2
export namespace=$3
export notify_email=$4
export log_file=$5
...
if [[ ${notify_email} =~ $email_regex ]] ; then
  ...
  # SMTP settings
  SMTP_USER="ipmcloud.icp@gmail.com"
  SMTP_PASS="c7d4540b72c44a30a72ae9f698062488"
  SMTP_SERVER="smtps://smtp.gmail.com:465"
  MAIL_FROM="ipmcloud.icp@gmail.com"
  
  CONSOLE_URL=$(oc get route -n "${namespace}" "${namespace}"-navigator-pn -o json | jq -r .spec.host)
  
  OPENSHIFT_URL=$(oc get route console -n openshift-console | grep 'console' | awk '{print $2}')
  
  if [ -z $CONSOLE_URL ]
  then
	CONSOLE_URL=$OPENSHIFT_URL
  ...
  STATEMENT="Installation of ${release_name} completed successfully on $(date '+%m/%d/%Y') $(date +"%r %Z"). <br> Your $COMPONENT URL is https://${CONSOLE_URL}"
  STATUS_LOGO="&#9989;"
            ...
            <td align='center' valign='top' style='border-collapse: collapse; border-spacing: 0; margin: 0; padding: 0;'
                ...
                <table border='0' cellpadding='0' cellspacing='0' align='center' bgcolor='#FFFFFF' width='660'
                    ...
                    <tr style='background-color: #1C1C1C; background: linear-gradient(90deg, #003b68 20%, #1C1C1C 100%);height: 75px'>
                        <td align='center' valign='top' style='border-collapse: collapse; border-spacing: 0; margin: 0; padding: 0; padding-left: 6.25%; padding-right: 6.25% ;width: 87.5%; font-size: 24px; font-weight: 400; line-height: 130%; padding-top: 25px; color: #fff; font-family: sans-serif;'>IBM Cloud Pak for Integration</td>
                    </tr>
                            ...
                            <table align='center' border='0' cellspacing='0' cellpadding='0' style='width: inherit; margin: 0; padding: 0; border-collapse: collapse; border-spacing: 0;'>
                                <tr>
                                    ...
                                    <td align='left' valign='middle' style='font-size: 17px; font-weight: 400; line-height: 160%; border-collapse: collapse; border-spacing: 0; margin: 0; padding: 0; padding-top: 25px; color: #000000; font-family: sans-serif;'>
                                        <b style='color: ${STATEMENT_COLOR};  font-size: 1.3em;'>Notification from CONATI4CP <br> [Cognizant Native Installer for Cloud Paks] !</b><br /><br />
